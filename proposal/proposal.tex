...
%Its usefulness has been applied to a host of language features.



%The usefulness of the family of CPS transformations can hardly be overstated.
% it clarifies complex, intricate control flow
% 

% I can't write a coherent sentence today so here are notes for my recovery:
% The paragraph above is a good place to transition into modelling language features with CPS transformations. That isn't the focus of the proposal, though, which is to provide a meaning-preserving transformation which itself could be used for a host of purposes. Really, the point is to keep related work in its section.
% The project description should reiterate the proposal, introduce the cm language and its semantics, mention the composition of the lambda calculus, and thoroughly present the proposal (whatever that means). It would be great to transition naturally into
% The validation section presents the theorem which will be proven. Despite its location, the project includes a proof of correctness of the transformation along with its presentation.


%\subsection{CPS transformations}

%CPS transformations are commonly employed to ...

%Pettyjohn et al. \cite{pettyjohn2005continuations} provide a semantics

%CPS transformations have been thoroughly studied
...
% the language grammar is given by E and F

We will provide a CPS-like language transformation and verify that it preserves program meaning. A language transformation transforms programs in a source language into semantically equivalent programs in a target language.

The definition/syntax/grammar of the source language, $\lambda_{cm}$, is given in figure \ref{language-syntax}. Its terms are those of the $\lambda$-calculus--single variables, $\lambda$-abstractions, and applications--along with forms to install a mark on the continuation (\emph{wcm} short for \emph{with-continuation-mark}) and retrieve the continuation's marks (\emph{ccm} short for \emph{current-continuation-marks}).\footnote{Notice that \emph{wcm} and \emph{ccm} have one parameter fewer than the forms introduced earlier. This is because $\lambda_{cm}$ expresses unkeyed marks. While it can be shown that a language with keyed marks can be expressed in terms of a language with unkeyed marks, such as $\lambda_{cm}$, this is not our concern.}

In order to preserve proper tail-call behavior, directly nested \emph{wcm} forms evaluate to the innermost one; the outer forms are discarded. 
