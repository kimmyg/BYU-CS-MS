...
Behavioral semantics mitigate this problem somewhat with a different approach. Rather than expressing an effect or property of the program, behavioral semantics express the execution of the program on an abstract machine.

% why are behavioral semantics not modular enough?

Monadic semantics provide a modular approach to precisely specifying language meaning. This type of semantics is so termed because of the appeal to the \emph{monad}, a concept stemming from category theory. Moggi \cite{moggi1989computational} applies monads as a categorical semantics of computational models such as non-termination and non-determinism. To illustrate this, we require the following definitions:
...
% talk about the semantics a bit
