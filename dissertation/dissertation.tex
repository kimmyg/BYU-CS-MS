...
% define pair, false and nil in terms of church encodings
...
% C[(e0 e1)] -> C[(* e1)][C[e0]] ->* C[(* e1)][C[v0]] -> C[(v0 e1)] -> C[(v0 *)][C[e1]]
% ->* C[(v0 *)][C[v1]] -> C[(v0 v1)] ->* C[v2]

% show that (wcm e0 e1) -> v means that C[(wcm e0 e1)] -> C[v]
% we're showing C[(wcm v0 e1)] k (flag,vs) -> C[v] => C[e1] k (true,v0:vs) -> C[v]

% C[(wcm v0 e1)] k (flag,vs) = C[e1] k (true,v0:vs)
% C[(ccm)] k (flag,vs) = vs = C[ms]
% ? => ( (wcm v0 e1) -> v1 => C[(wcm v0 e1)] -> C[v1])
% (wcm v0 e1) -> v1 and C[(wcm v0 e1)] k (flag,ms) -> C[v1] <=> C[e1] k (true,v1:ms') -> C[v1] 


% we want to show that (wcm 1 (ccm)) ->* [1] => C[(wcm 1 (ccm)] k (false,[]) ->* C[[1]]
% 

% C[(wcm 1 (wcm 2 (ccm)))] k (false,[])=C[(wcm 1 *)][C[(wcm 1 (wcm 2 (ccm)))] k] 
% at the bottom, we have C[E[(ccm)]] k (false,[])-> chi(E) => C[(ccm)] k (true,[2,1]) -> [2,1]

% basically, we compute the chi metafunction as we go
% we can show we follow the rules
% \x.e v -> e[x<-v]
% (wcm v v') -> v'
% (wcm v (wcm v' e)) -> (wcm v' e)
% (ccm) -> chi(E)
...
% structural induction on contexts
% case E=*, snd m = C[nil] = C[chi(E)]
% case E=wcm v E'

% prove that the marks argument is the evaluation of the chi metafunction

% for every program p, the evaluations are the same
% p -> v => C[p] -> C[v]
% proof
% p is of the form (e0 e1), (wcm e0 e1), (ccm), \x.e, or x
% case (e0 e1): (e0 e1) -> v1 => C[(e0 e1)] -> C[v1]
% assume e0 -> v0 => C[e0] -> C[v0], e1 -> v1 => C[e1] -> C[v1]
% show 

% case (wcm e0 e1)
% C[(wcm e0 e1)] -> C[(wcm * e1)][C[e0]] ->* C[(wcm * e1)][C[v0]] -> C[(wcm v0 e1)]
% -> C[
...
% now consider the context (continuation) in which an expression is evaluated
% what we'll do is inductively strip away the continuation in layers
% suppose we have (wcm e1 (wcm e2 (e3 e4) e5))
% we show that C[(wcm v1 *)[(wcm e2 (e3 e4) e5)]] is the same as C[(wcm v1 *][C[(wcm e2 (e3 e4) e5)]]
% C[E1[E2[E3[e]]]] <= C[E1][C[E2[E3[e]]]] <= C[E1][C[E2][C[E3[e]]] <= C[E1][C[E2][C[E3][C[e]]]]
% (of course, these aren't static)
% as in, if we can show C[E[e]]=C[E][C[e]], we can show the above
...
% for E=*, it is trivial since C[*]=*
% for E=* e2, e=e1, show that C[e1 e2]=C[* e2][C[e1]]
% we can argue this by virtue of the recursive nature of the transform

% reason about (wcm (\x.x 2) (\y.y (ccm)))
% (wcm 2 (\y.y (ccm)))
% (wcm 2 (\y.y \p.((p 2) \x.\y.y)))
% (wcm 2 \p.((p 2) \x.\y.y))
% \p.((p 2) \x.\y.y)

% C[(wcm (\x.x 2) (\y.y (ccm)))] =
% C[(wcm * (\y.y (ccm)))][C[(\x.x 2)]] -> 
% C[(wcm * (\y.y (ccm)))][C[2]] =
% C[(wcm 2 (\y.y (ccm)))] =
% C[(wcm 2 *)][C[(\y.y (ccm))]] =
% C[(wcm 2 *)][C[(\y.y *)][C[(ccm)]]] ->
% C[(wcm 2 *)][C[(\y.y *)][C[\p.((p 2) \x.\y.y)]]] (why?) =
% C[(wcm 2 *)][C[(\y.y \p.((p 2) \x.\y.y))]] =
% C[(wcm 2 *)][C[\p.((p 2) \x.\y.y)]] =
% C[(wcm 2 \p.((p 2) \x.\y.y)] =
% C[\p.((p 2) \x.\y.y)]

% need to show that C[E[(ccm)]]=C[E][C[(ccm)]]?
% C[E1[E2[(ccm)]]]=C[E1][C[E2[(ccm)]]]=C[E1][C[E2][C[(ccm)]]]
% no, this is begging the question

% show compositionality and "simulation"?
% simulation: C[e1] -> C[v1], C[e2] -> C[v2] => (e1 e2) -> (v1 v2) or
% (e1 e2) -> v1 => C[(e1 e2)] -> C[v1] (which of these two?)



% define transformation of contexts
% take fischer's CPS as an example
% C[x] = \k.(k x)
% C[\x.M] = \k.(k \x.C[M])
% C[(M N)] = \k.(C[M] (\m.(C[N] (\n.((m n) k)))))

% C[*] = *
% C[E e] = \k.(C[E] (\v.(C[e] (\e'.((v e') k))))
% C[v E] = \k.(C[v] (\v'.(C[E] (\e.((v' e) k))))

% we have that C[E[e]]=C[E][C[e]]

% E[(e1 e2)] -> E[v3] => C[E[(e1 e2)]] -> C[E[v3]]
% proof
% assume e1 -> v1 => C[e1] -> C[v1] and e2 -> v2 => C[e2] -> C[v2]
% C[(e1 e2)] = C[(* e2)][C[e1]] -> C[(* e2)][C[v1]] = C[(v1 e2)] = 
% C[(v1 *)][C[e2]] -> C[(v1 *)][C[v2]] = C[(v1 v2)]
% then we evaluate abstractly to show that (v1 v2) -> v3 => C[(v1 v2)] => C[v3]
% Is this specifically for the first rule in the semantics or for the general
% proof about the transform?


% for all E, E[(ccm)] -> E[v] => C[E[(ccm)]] -> C[E[v]]
% proof
% 

% for all p in lambda-cm, p -> v => C[p] -> C[v]
% p is of the form (e1 e2), (wcm e1 e2), (ccm), v or x
% x goes to error
% v is trivial

% show that C[E[(wcm v1 (wcm v2 e1))]] -> C[E[(wcm v2 e1)]]?

% at some level, we need to show that (false,C[ms]) -> (true,C[v:ms]) and (true,C[m:ms]) -> (true,C[v:ms])
% and that (ccm) yields C[ms]
% how about this: show that given (wcm v1 e1) and (flag,ms), it's equivalent to e1 (true,v1:ms')
% where ms' is correct (do by case analysis on flag)
% for (wcm e1 e2), assume e1 (false,ms) is correct by induction and -> v1
% by C[(wcm * e2)][C[e1]]
% for (e1 e2), assume e1 (false,ms) is correct by induction and -> v1
% for (

% establish that e1 -> v1 and e2 -> v2 and (v1 v2) -> v3 => C[(e1 e2)] -> C[v3] (by structural induction and abstract reduction)


% case C[e] = C[(wcm e1 e2)] given k, (flag,vs)
% C[(wcm * e2)][C[e1]], assume by induction C[e1] -> C[v1] is correct given k', (false,vs)
% C[(wcm v1 *)][C[e2]]
% if flag, assume by induction that (wcm  
%\chapter{Conclusion}
