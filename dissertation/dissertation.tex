...
due to Fischer \cite{fischer1972lambda}. Before doing so, we offer a brief primer on the 
relevant parts of the $\lambda$-calculus.
...
application. There is, however, yet more subtlety of which we must be aware, namely, in which contexts we allow applications to be reduced. For example, in the call-by-value $\lambda$-calculus (so named because arguments are reduced to values before application), $\lambda y.y\,x$ is reducible; by contrast, $\lambda x.(\lambda y.y\,x)$ is not. There are regimes--call-by-name, for example--in which reduction beneath abstractions is legal. In a call-by-name reduction regime, the latter term, when applied to $z$, would reduce as
\begin{align*}
...
Recall that terms in the $\lambda$-calculus take the form of lone variables $x$, $\lambda$-abstractions $\lambda x.M$, and applications $M\,N$ where $M$ and $N$ are themselves $\lambda$-calculus terms. A comprehensive definition need only specify transformations for these three categories.
...
$\lambda$-calculus--abstractions--are contaminated by the transform. For example, consider 
the transformation of $\lambda x.x$
\[
