...
the token \scheme{undefined}. The category \scheme'e' is defined to contain the expression 
...
By induction, we assume that \scheme{e_1} reduces to exactly one value. Then the ``with''-rule
can only be applied once, resulting in a single term \scheme{e_2} in \scheme'e' which, by our
...
Define \scheme{TRUE}=\scheme|(lambda (x) (lambda (y) x))|, \scheme{FALSE}=\scheme|(lambda (x) (lambda (y) y))|, \scheme|(PAIR a b)|=\scheme|(lambda (z) ((z a) b))|, \scheme{FST}=\scheme|(lambda (p) (p TRUE))|, \scheme{SND}=\scheme|(lambda (p) (p FALSE))|, and \scheme{NIL}=\scheme{FALSE}.
