...
\]
From here, there are two contingencies for which we must account:
\begin{enumerate}
\item The body of $\mathcal{C}[e_0]$ evaluates to \scheme'error', as in app8-e0-error.
The entire term evaluates to \scheme'error', by app9-e0-error.
\item The body of $\mathcal{C}[e_0]$ fails to terminate, as in app8-e0-bottom.
The entire term fails to evaluate, by app9-e0-bottom.
\end{enumerate}
% assume C[e0] goes to correct value (structural induction) E[(* e1)][e0] -> E[(* e1)][v0]
Otherwise, we assume the body of $\mathcal{C}[e_0]$ evaluates to $\mathcal{C}'[v_0]$, as in app8.
...
\]
From here, we must account for the same contingencies as before:
\begin{enumerate}
\item The body of $\mathcal{C}[e_1]$ evaluates to \scheme'error', as in app10-e1-error.
The entire term evaluates to \scheme'error', by app11-e1-error.
\item The body of $\mathcal{C}[e_1]$ fails to terminate, as in app10-e1-bottom.
The entire term fails to evaluate, by app11-e1-bottom.
\end{enumerate}
% assume C[e1] goes to correct value (structural induction) E[(v0 *)][e1] -> E[(v0 *)][v1]
Otherwise, we assume the body of $\mathcal{C}[e_1]$ evaluates to $\mathcal{C}'[v_1]$, as in app10.
...
\begin{align*}
(((\mathcal{C}'[v_0]\,\mathcal{C}'[v_1])\,\mathcal{C}[E])\,\overline{((\mathbf{pair}\,\xi(E))\,\chi(E))}) &=((\mathcal{C}[e'_0[x\leftarrow v_1]]\,\mathcal{C}[E])\,\overline{((\mathbf{pair}\,\xi(E))\,\chi(E))})\\
% assume (v0 v1) -> e2
% -> ((C[e2] C[E]) ((pair xi(E)) C[chi(E)]))
                                                                                                          &\rightarrow_{\lambda_v}((\mathcal{C}[e_2]\,\mathcal{C}[E])\,\overline{((\mathbf{pair}\,\xi(E))\,\chi(E))})
\end{align*}
for some $e_2$.
...
\begin{align*}
% prove that *[e] -> v => ((C[e] C[*]) ((pair xi(*)) C[chi(*)])) -> C[v]
% ((C[e] C[*]) ((pair xi(*)) C[chi(*)])) -> 
\end{align*}
\end{proof}

\begin{proof}[Case $E=\bullet$, $e=(\mathrm{ccm})$]
$(\mathcal{C}[\bullet]\,\mathcal{C}'[\chi(E)])$
\end{proof}

\begin{proof}[Case $E=E'(\bullet\,e_1)$, $e=(\mathrm{ccm})$]
$(\mathcal{C}[\bullet]\,\mathcal{C}'[\chi(E)])$
\end{proof}
