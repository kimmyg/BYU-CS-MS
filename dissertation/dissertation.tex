...
\]
Once again, we assume that the property holds for $E$ and $F$. Then $e$ and $f$ are
irreducible. In order to conclude that the property holds for the application, we must
establish that the application an abstraction to an irreducible term ultimately yields an 
irreducible term. The key lies in a case analysis of the operator. It must be either a 
variable or an abstraction.

First, consider the application of a variable to a variable.
\[
x y
\]
This cannot be reduced directly; hence, it is irreducible.
\begin{align*}
...
Randomized testing can increase our confidence in various assertions but are no substitute 
for proof.


While we will not provide it, the proof of this theorem would proceed inductively on the 
structure of terms in the language.
