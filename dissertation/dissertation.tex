...
$E$, does not define abstract syntax but instead reduction contexts. A reduction context,
according to this definition, is either a hole (a special token in Redex) which is the
destination of a result [say that better] or ... various forms which dictate evaluation
order.
...
This term defines the reduction relation of the \emph{toy} language. The five defined
...
abstracted, requiring an argument encoding the current continuation marks. We will not 
visit the other approaches and will instead assume that our current approach is a fine way 
