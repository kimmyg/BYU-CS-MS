...
constitutes values, and we appeal to this.}
\end{toythm}
...
\]
then $v\equiv v'$. We make a weaker assertion here that the terms are equivalent and not 
that the terms are equal. This is not to say that the terms should have syntactic 
equivalence or even $\alpha$-equivalence, but only that we should be able to formally 
relate them.

Next, consider the program $ccm$, perhaps the simplest program strictly in $\lambda_{cm}$. 
...
The tail marks flag is correct meaning iff the context is wcm v F, the flag will be true
Proof
The beginning of evaluation (hole, here?) passes false, which is correct.
E e passes false
v E passes false
...
show behavior for wcm v (wcm e e) is correct (thus showing (wcm v (wcm v' e)) -> wcm v' e)
