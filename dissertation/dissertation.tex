...
% C[e] C[E] (xi(E),chi(E))
% C[(e0 e1)] C[E] (xi(E),chi(E))
% C[e0] \e.((Ce1 \f.(((e f) C[E]) (xi(E),chi(E))) (false,chi(E))) (false,chi(E))
% C[e1] \f.(((C'[v0] f) C[E]) (xi(E),chi(E))) (false,chi(E))
% assume C'[v0]=\x.C[e0'] so C[e0'][x <- C'[v1]]
% do something to show that C[e0'][x <- C'[v1]] equiv C[e0'[x <- v1]]?

% values are of the form \x.C[e]

% ((C[p] \v.C[v]) ((pair false) nil))
% ((C[(e0 e1)] \v.C[v]) ((pair false) nil)) ->
% ((C[e0] \e.((C[e1] \f.(((e f) \v.C[v]) ((pair false) nil))))) ((pair false) nil))
% ((C[e1] \f.(((\x.C[e0'] f) \v.C[v]) ((pair false) nil))))
% (((\x.C[e0'] \y.C[e1']) \v.C[v]) ((pair false) nil))
% ((C[e0'][x <- \y.C[e1']] \k.C[v]) ((pair false) nil))
% ((C[e0'[x <- v1]] \k.C[v]) ((pair false) nil))

% (fact-tr 3 1)
% (if (= 3 0) 1 (wcm 3 (fact-tr 2 (* 3 1))))
% (wcm 3 (fact-tr 2 (* 3 1)))
% (wcm 3 (fact-tr 2 3))
% (wcm 3 (if (= 2 0) 1 (wcm 2 (fact-tr 1 (* 2 3)))))
% (wcm 3 (wcm 2 (fact-tr 1 (* 2 3))
% (wcm 2 (fact-tr 1 (* 2 3)))
% (wcm 2 (fact-tr 1 6))
% (wcm 2 (if (= 1 0) 1 (wcm 1 (fact-tr 0 (* 1 6)))))
% (wcm 2 (wcm 1 (fact-tr 0 (* 1 6))))
% (wcm 1 (fact-tr 0 (* 1 6)))
% (wcm 1 (fact-tr 0 6))
% (wcm 1 (if (= 0 0) 6 (fact-tr (- 0 1) (* 0 6)))
% (wcm 1 6)
% 6
...
% The crux of this proof comes down to two invariants: first, every expression is 
% passed the value of chi(E) before evaluation; second, every expression is passed
% a flag indicating whether it is the body of a wcm expression. We need to prove 
% that these truly are invariant.

% To do this, we consider the transformation of an evaluation context and prove a
...
% We then demonstrate on the following fact:

% if E[e] -> E[v], then C[E[e]] -> C[E[v]] by C[e] k (xi(E),chi(E)) -> C[v] k (xi(E),chi(E))

% We do this by C[E[e]] -> C[E][C[e]] -> C[E][C[v]] -> C[E[v]]

% Consider the following program:

% (wcm (\x.x 1) (wcm (ccm) (\x.x (ccm))))
% E=*, e=(wcm (\x.x 1) (wcm (ccm) (\x.x (ccm))))
% E[e] -> E[v] => C[E[e]] -> C[E[v]]
% This is simple: C[E] = *; C[e] = C[(wcm (\x.x 1) (wcm (ccm) (\x.x (ccm))))]
% by composability of C[*][C[e]] as C[*[e]]
% C[(wcm (\x.x 1) (wcm (ccm) (\x.x (ccm))))] k0 (false,chi(*))
% C[(wcm * (wcm (ccm) (\x.x (ccm))))] k0 (false,chi(*)) C[(\x.x 1)] 




% look at C[(e0 e1)] k' (xi(E),chi(E))
% by induction, E(* e1)[e0] ->cm E(* e1)[v0] => C[e0] k0 (false,chi(E0)) -> (k0 v0)
% where E0 = E(* e1)
% by induction, E(v0 *)[e1] ->cm E(v1 *)[v1] => C[e1] k1 (false,chi(E1)) -> (k1 v1)
% where E1 = E(v0 *)

% with v0 = \x.e0'

% C[(wcm v0 e1)] k0 (false,vs) equiv C[e1] k0 (true,v0:vs)
% C[(wcm v0 e1)] k0 (true,v:vs) equiv C[e1] k0 (true,v0:vs)
% C[(wcm e0 e1)]

% E[e0'[x<-v1]] ->cm E[e3] => C[(\x.e0' v1)] k' (xi(E),chi(E)) ->v C[


% an ML-esque interpreter
% eval p = interp p False []

% interp (e0 e1) flag ms = let v0 = interp e0 false ms in
%                            case v0 of
%                              error -> error
%                              (\ (x) e2) -> let v1 = interp e1 false ms in
%                                case v1 of 
%                                  error -> error
%                                  (\ (y) e3) -> interp (subst e2 x (\ (y) e3)) flag ms

% interp (wcm e0 e1) flag ms = let v0 = interp e0 false ms in
%                                case v0 of                               
%                                  error -> error
%                                  otherwise -> if flag
%                                                 let (v:vs) = ms in
%                                                   interp e1 True (v0:vs)
%                                               else
%                                                 let vs = ms in
%                                                   interp e1 True (v0:vs)

% interp (ccm) flag ms = ms
% interp v flag ms = v
% interp x flag ms = error
...
% assume e0 -> v0 = \x., e1 -> v1
\end{proof}

\begin{lemma}
For all contexts $E$, $\xi(E)$ is correct.
\end{lemma}

We proceed with a proof by induction over $E$.

\begin{proof}[Case $\bullet$]
If $E=\bullet$
\end{proof}

\begin{proof}[Case $(\bullet\,e_1)$]
Assume it holds for $E$. $\chi(E[(\bullet\,e_1)])=\chi(E)$.
\end{proof}

\begin{proof}[Case $(e_0\,\bullet)$]
Assume it holds for $E$. $\chi(E[(e_0\,\bullet)])=\chi(E)$.
\end{proof}

\begin{proof}[Case $(\mathrm{wcm}\,\bullet\,e_1)$]
Assume it holds for $E$. $\chi(E[(\mathrm{wcm}\,\bullet\,e_1)])=\chi(E)$.
\end{proof}

\begin{proof}[Case $(\mathrm{wcm}\,v_0\,\bullet)$]
Assume it holds for $E$ where $E\ne E'[(\mathrm{wcm}\,v\,\bullet)]$ for any $E'$ or $v$.
$\chi(E[(\mathrm{wcm}\,v_0\,\bullet)])=v_0 :\chi(E)$.
\end{proof}

\begin{lemma}
For all contexts $E$, $\chi(E)$ is correct.
\end{lemma}

We proceed with a proof by induction over $E$.

\begin{proof}[Case $\bullet$]
In the base case, it is correct.
\end{proof}
...
\begin{proof}[Case $(\mathrm{wcm}\,v\,(\mathrm{wcm}\,v_0\,\bullet))$]
Assume it holds for $E$. 
$\chi(E[(\mathrm{wcm}\,v\,(\mathrm{wcm}\,v_0\,\bullet))])=v_0 :\chi(E)$.
\end{proof}
...
Let $e_0\equiv_v e_1$ signify there exists $e'$ such that $e_0\rightarrow_{\lambda_v}^{*}e'$ and $e_1\rightarrow_{\lambda_v}^{*}e'$.

\begin{lemma}
For all contexts $E$, $((\mathcal{C}[E[e]]\,k')\,((\mathbf{pair}\,\mathbf{false})\,\mathbf{nil}))\equiv((\mathcal{C}[e]\,k'')\,((\mathbf{pair}\,\xi(E))\,\chi(E)))$ for some continuation $k''$.
\end{lemma}

We proceed by induction on the structure of $E$.

\begin{proof}[Case $\bullet$]
If $E=\bullet$, $\xi(E)=\mathbf{false}$.
\end{proof}
...
\end{maintheorem}
A program \scheme'p' must take the form \scheme|(e_0 e_1)|, \scheme|(wcm e_0 e_1)|,
\scheme|(ccm)|, \scheme{v_0}, or \scheme'x'. We will consider each case.

% C[(\x.x 1)] k (false,nil) -> C[\x.x] k0 (false,nil) -> C[1] k1
% k /is/ the context (continuation)
% encoding of * is \v.\k.\m.(k v)
% encoding of (* e1) is
% \e.((C[e1] \f.(((e f) C[E]) (xi(E),chi(E))) (false,chi(E)))
% where k is the encoding of the parent context
% encoding of (v1 *) is
% \f.(((C'[v0] f) k) (xi(E),chi(E)))
% encoding of (wcm * e1) is
% \v.(\k.(k ((((chi(E) \x.x) \x.x) \x.C[\y.y]) \v.C[v]) \x.x)
%  \t.(\k.(k xi(E)))
%   ...
% encoding of (wcm v0 *)
% C[E] (tail!)
...
% C[*] = \k.\m.(k *)
% C[((e0 e1) e2)] = C[e0] C[((* e1) e2)] ... 
% \e.((C[e1] \f.(((e f) \e.((C[e2] \f.(((e f) C[E]

% C[(e0 e1)] C[*] (xi(E),chi(E)) ->
% C[e0] C[E] (xi(E),chi(E))
% if E[e0] -> E[v0], then C[e0] C[E] (xi(E),chi(E)) -> C[v0] C[E] 

%C[(wcm 1 (ccm))] k (false,nil) = C[*[(wcm 1 (ccm))]] k (xi(*),chi(*)) ->* 
%C[1] k0 (false,[])
%C[(ccm)] k (true,[1])


% C[(e_0 e_1)] k0 chi(E) -> C[e'_0[x <- v1]] k0 chi(E)
% C[(wcm v0 (wcm e1 e2))] k0 chi(E) -> C[v2] k0 chi(E') where E[(wcm v0 (wcm v1 e2))] -> E[(wcm v1 e2)] -> 


% suppose we have E[(e_0 e_1)]
% C[E[(e_0 e_1)]] k0 (false,nil) equiv C[(e_0 e_1)] k0 (\xi(E),chi(E))
% -> C[e2] k0 (\xi(E),chi(E))

% C[E[(ccm)]] k0 (false,nil) equiv C[(ccm)] k0 (\xi(E),chi(E))
%by induction

% C[(wcm e0 e1)] k0 (false,nil)
% C[e0] k1 (false,nil) -> v1
% C[(wcm v1 e1)] k0 (false,nil)
% C[(wcm v1 *)[e1]] k0 (false,nil)
% C[e1] k0 (true,[v1])

% if   C[E[(wcm e0 e1)]] k0 (false,nil) equiv C[(wcm e0 e1)] k0 (false,chi(E))
% then C[E(wcm * e1)[e0]] k0 (false,nil) equiv C[e0] k1 (true,chi(E')) where E'=E(wcm * e1)
% and  C[E(wcm v1 *)[e1]] k0 (false,nil) equiv C[e1] k0 (true,chi(E'')) where E''=E(wcm v1 *)

\begin{proof}[Case \scheme|(e_0 e_1)|]

%$((\mathcal{C}[E(e_0\,e_1)]\,k_0$

%if E[e0] -> E[v0] then C[E[e0]]=C[E][C[e0]]=C[E][C[v0]]=C[E[v0]]

%C[E[(wcm v0 e1)]] k0 chi(E')\equiv
...
% if E[(ccm)] -> E[chi(E)], then C[E[(ccm)]] ? ? equiv C[(ccm)] ? (\xi(E),chi(E))

% 

% by inductive hypothesis, C[E[(ccm)]] k (false,nil)
